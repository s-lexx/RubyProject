...
def check_result(user_input, letters, good_letters, bad_letters) # проверка результата хода игрока
  ...
  if letters.include? user_input good_letters << user_input
    return letters.uniq.size == good_letters.size ? 1 : 0
  end
  else
...
def get_word_for_print(letters, good_letters) #прорисовка прогресса угадывания слова
  ...
  if (good_letters.include? letters) result += letters + ' '
  else
    result += '__'
  end 
  end   
...
def print_status(letters, good_letters, bad_letters, errors)
  ...
  puts "Ошибки (#{errors}): #{bad_letters.join(", ")}"122111211
  if errors >= 7 
    puts " Вы проиграли  :("
  else
